...
\begin{document}
	...
	Now, we will show that the computed $\widehat{K}_{N,n,m}$ by our S-EDMD method converges not only in large data $m$ and large dictionary size $N$, but also in zero-limit of sampling time $t=1/n$.
	...
	Now we have Koopman matrix approximation at discrete time $K_{N,n}(1/n)$. We will use it to construct the matrix$A_{N,n}$, which is the representation of the discrete projected Koopman generator approximant $\mathcal{A}_{N,n}$. Then we find its limit in strong operator topology as $n\to\infty$. 
	...
	\end{remark}
	Define the matrix $A_N \coloneqq \Psi_X^{\dagger}\mathcal{L}\Psi_X$. Choose $f \in \textrm{span}\{\psi_i\}_{i=1}^N$ so that $f = \mathbf{\Psi}_N\mathbf{a}$. Then we have
	\begin{align*}
		&\quad \mathbf{\Psi}_N A_{N,n}\mathbf{a} \\
		&= \mathbf{\Psi}_N\left(\Psi_X^{\dagger}\mathcal{L}\Psi_X + O(\tfrac{1}{n})\right)\mathbf{a} \\
		&= \mathbf{\Psi}_N\left(\Psi_X^{\dagger}\mathcal{L}\Psi_X\right)\mathbf{a} + \mathbf{\Psi}_N O(\tfrac{1}{n})\mathbf{a} \quad (\text{Here the $O(\tfrac{1}{n})$ is a matrix}) \\
		&= \mathbf{\Psi}_N A_N \mathbf{a} + \mathbf{\Psi}_N O(\tfrac{1}{n})\mathbf{a}
	\end{align*}
	...
	$$ 
	with $A_N \coloneqq \Psi_X^{\dagger}\mathcal{L}\Psi_X$.	In other words,
	$$
