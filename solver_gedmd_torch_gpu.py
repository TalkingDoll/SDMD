...
class KoopmanModelTorch(nn.Module):
    def __init__(self, dict_net, target_dim, k_dim):
        ...
        self.k_dim = k_dim
        self.layer_L = nn.Linear(k_dim, k_dim, bias=False)
        #self.layer_L.weight.requires_grad = False
    
    def forward(self, input_x, input_dpsi_x):
        psi_x = self.dict_net.forward(input_x)
        # output is difference between dPsi_X and PsiX*L
        psi_x_l = self.layer_L(psi_x)
        outputs = input_dpsi_x - psi_x_l
        return outputs
...
class KoopmanSolverTorch(object):
    ...
    def __init__(self, dic, target_dim, reg=0.0, checkpoint_file='example_koopman_net001.torch', fnn_checkpoint_file= 'example_fnn001.torch', 
                ...
                a_b_file= None, generator_batch_size= 4, fnn_batch_size= 32):
        """Initializer
    ...
    def compute_final_info(self, reg_final):
        ...
        data_x = torch.DoubleTensor(self.data_x_train).to(device)
        data_y = torch.DoubleTensor(self.data_y_train).to(device)
        
        # psi_x = dic(data_x)
        # psi_y = dic(data_y)
        
        # Compute Psi_X and Psi_Y
        self.Psi_X = self.dic(data_x)
        self.Psi_Y = self.dic(data_y)
        data_x_train_tensor= torch.DoubleTensor (self.data_x_train)
        self. L_Psi = self.compute_generator_L(data_x_train_tensor, self.b_Xt, self.a_Xt)
        self.L_np = self.L_Psi.detach().cpu().numpy()
        self.eig_decomp(self.L_np)
        #self.compute_mode()

    def eig_decomp(self, L):
        """ eigen-decomp of K """
        self.eigenvalues, self.eigenvectors = la.eig(L)
    ...
    def build_model(self):
        self.koopman_model = KoopmanModelTorch(dict_net=self.dic, target_dim=self.target_dim, k_dim=self.L_Psi.shape[0]).to(device)
        
    def fit_koopman_model(self, koopman_model, koopman_optimizer, checkpoint_file, xx_train, dpsi_x_train, xx_test, dpsi_x_test,
                      batch_size=32, lrate=1e-4, epochs=1000, initial_loss=1e15):
        load_best = False
        xx_train_tensor = torch.DoubleTensor((xx_train)).to(device)
        # dpsi_x_train_tensor = torch.DoubleTensor((dpsi_x_train)).to(device)
        xx_test_tensor = torch.DoubleTensor((xx_test)).to(device)
        # dpsi_x_test_tensor = torch.DoubleTensor((dpsi_x_test)).to(device)
        #print ('type of xx train: ', type (xx_train))
        #xx_train_tensor = xx_train #torch.DoubleTensor((xx_train)).to(device)
        dpsi_x_train_tensor =dpsi_x_train # torch.DoubleTensor((dpsi_x_train)).to(device)
        #xx_test_tensor = xx_test#torch.DoubleTensor((xx_test)).to(device)
        dpsi_x_test_tensor = dpsi_x_test #torch.DoubleTensor((dpsi_x_test)).to(device)
        train_dataset = torch.utils.data.TensorDataset(xx_train_tensor, dpsi_x_train_tensor)
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=batch_size, shuffle=False)
    
        val_dataset = torch.utils.data.TensorDataset(xx_test_tensor, dpsi_x_test_tensor)
        ...
        if load_best:
            ...
            mlp_mdl.layer_L.requires_grad = False
    ...
    def train_psi(self, koopman_model, koopman_optimizer, epochs, lr, initial_loss=1e15):
        ...
        dPsi_X_train = self.compute_dPsi_X(torch.DoubleTensor(self.data_x_train).to(device), self.b_Xt, self.a_Xt).detach()
        dPsi_X_val = self.compute_dPsi_X(torch.DoubleTensor(data_x_val).to(device), self.b_Xt, self.a_Xt).detach()
        psi_losses, best_psi_loss = self.fit_koopman_model(self.koopman_model, koopman_optimizer, self.checkpoint_file, self.data_x_train[:-1,:],
                                                      dPsi_X_train, data_x_val[:-1, :], dPsi_X_val, self.batch_size,
    ...
    def compute_generator_L(self, data_x, b_Xt, a_Xt,  lambda_reg=0.01):
        # Compute dPsi_X
        data_x= data_x.to(device)
        dPsi_X = self.compute_dPsi_X(data_x.to(device), b_Xt, a_Xt)
        self.dPsi_X= dPsi_X
    ...
    def build_with_generator(self, data_train, data_valid, epochs, batch_size, lr, log_interval, lr_decay_factor):
        """Train Koopman model and calculate the final information,
        such as eigenfunctions, eigenvalues and L.
        For each outer training epoch, the koopman dictionary is trained
        by several times (inner training epochs), and then compute matrix K.
        Iterate the outer training.
        ...
        self.batch_size = batch_size
        data_x_train_tensor= torch.DoubleTensor (self.data_x_train)
        # here we load precomputed drift and diffusion coefficent if the joblib file is specified.
        #otherwise we obtain them by training feed-forward neural networks
        if  (self.a_b_file is not None):
            a_Xt_np, b_Xt_np= joblib.load(self.a_b_file)
            self.a_Xt= torch.DoubleTensor (a_Xt_np).to (device)
            self.b_Xt= torch.DoubleTensor (b_Xt_np).to (device)
            print ('Precomputed SDE coefficients loaded from:', self.a_b_file)
        else:
        ...
        # Build the Koopman DL model
        self.build_model()
        #initializing loss , learning rate 
        losses = []
        curr_lr = lr
        curr_last_loss = 1e15
        self.koopman_optimizer= torch.optim.Adam(self.koopman_model.parameters(), lr=lr, weight_decay=1e-5)
        for ii in arange(epochs):
            #starting outer epoch. In each outer epoch we compute generator L, set it as weight matrix of the
            #layer L of our Koopman NN. 
            #then we do several steps of training our NN that is the dictionary
            start_time = time.time()
            print(f"Outer Epoch {ii+1}/{epochs}")
            
            # One step for computing L 
            self. L_Psi = self.compute_generator_L(data_x_train_tensor, self.b_Xt, self. a_Xt)
            
            
            with torch.no_grad():
                self.koopman_model.layer_L.weight.data = self.L_Psi.detach()

            #  steps for training PsiNN, the number of inner epochs is given by epochs parameter below, here epochs= 4
            curr_losses, curr_best_loss = self.train_psi(self.koopman_model, self.koopman_optimizer, epochs=4, lr=curr_lr, initial_loss=curr_last_loss)
