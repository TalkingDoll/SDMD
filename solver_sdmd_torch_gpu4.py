...
class KoopmanSolverTorch(object):
    ...
    def compute_dPsi_X(self, data_x, b_Xt, a_Xt, delta_t):
        """
        Original compute_dPsi_X implementation removed.
        Compute LPsi_X using the Itô generator formula, equivalent to Ψ'(x):
        LPsi_X[i,j] = b[i,k] ∂_k ψ_j(x_i)
                    + ½ a[i,k,l] ∂^2_{k,l} ψ_j(x_i)
        Returns a tensor of shape (M-1, F)
        """
        # 1) Obtain gradients and Hessians directly (no batching needed, as vmap is used)
        jacobian, hessian = self.get_derivatives(data_x)

        # 2) Drop the last time point to align b_Xt and a_Xt
        M1 = b_Xt.shape[0]
        if jacobian.shape[0] == M1 + 1:
            J = jacobian[:-1]  # (M-1, F, D)
            H = hessian[:-1]   # (M-1, F, D, D)
        elif jacobian.shape[0] == M1:
            J = jacobian       # (M-1, F, D)
            H = hessian        # (M-1, F, D, D)
        else:
            raise ValueError(f"Sample count mismatch: jacobian {jacobian.shape[0]}, b_Xt {M1}")

        B = b_Xt  # (M-1, D)
        A = a_Xt  # (M-1, D, D)

        # 3) Compute drift and diffusion terms using einsum
        term1 = torch.einsum('mfd,md->mf', J, B)
        term2 = 0.5 * torch.einsum('mfkl,mkl->mf', H, A)

        # 4) Combine terms and return; do not multiply by delta_t
        LPsi_X = term1 + term2
        self.dPsi_X = LPsi_X
        return LPsi_X


    def get_derivatives(self, inputs, batch_size=None):
        """
        Return the gradient and Hessian for each sample and basis function:
        J: (M, F, D)
        H: (M, F, D, D)
        """
        # 1) Construct a function that computes gradient at a single point x: x -> (F, D)
        jac_fn = jacrev(self.dic)
        # 2) Construct a function that computes Hessian at a single point x: x -> (F, D, D)
        hess_fn = jacrev(jac_fn)

        # 3) Map them over the batch dimension using vmap
        J = vmap(jac_fn)(inputs)   # shape (M, F, D)
        H = vmap(hess_fn)(inputs)  # shape (M, F, D, D)
        return J, H
